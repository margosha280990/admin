...
if ($dbbuh){
	if (count($mas_dat)>0){
		for ($i=0, $coun=1; $i<count($mas_dat); $i+=2, $coun+=2){
			...
			$data = $mas_dat[$i];
			if ($date!=''){
					$dbbuh->query("UPDATE rosgid_reporttotax SET fackt_date='{$data}' WHERE id='{$date}'");
				...
				if ($sql_org_fin_repport->num_rows>0) {
					 while( $row = $sql_org_fin_repport->fetch_assoc()){
						...
						$dbbuh->query("INSERT INTO `rosgid_reporttotax`  VALUES (NULL, '{$rep_code}', '{$rep_name}', '{$id_rep}' ,'{$rep_type}', '{$pl_date}' , '{$data}')");
